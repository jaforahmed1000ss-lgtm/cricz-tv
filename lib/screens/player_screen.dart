import 'package:flutter/material.dart';
  ...
  class _PlayerScreenState extends State<PlayerScreen> {
    ...
    Future<void> _toggleFav() async {
      ...
      if (mounted) {
        ...
        await NotificationService.showFavoriteNotification(widget.channel.name, newState);
    ...
    void _shareChannel() {
      Share.share(
        'Watch ' + widget.channel.name + ' LIVE on CricZ TV!',
        subject: widget.channel.name + ' - CricZ TV',
      );
    }
    ...
    Future<void> _loadStream() async {
      ...
      try {
        await _player.stop();
        NotificationService.showStreamNotification(
          ...
          actions: [
            IconButton(
              icon: Icon(_isFav ? Icons.favorite_rounded : Icons.favorite_border_rounded,
                  color: _isFav ? const Color(0xFFFF4081) : Colors.white70, size: 22),
      ...
      return Stack(children: [
        Video(
          ...
          controls: (state) => _CricZControls(
            ...
            onBack: fullscreen ? () => _setFullscreen(false) : () => Navigator.pop(context),
          ),
        ),
        if (_loading && !_error)
          Container(
            ...
            child: Center(
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                const SizedBox(width: 48, height: 48,
                    child: CircularProgressIndicator(color: Color(0xFF00D2FF), strokeWidth: 3)),
                const SizedBox(height: 14),
                const Text('Loading stream...', style: TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.w600)),
                const SizedBox(height: 6),
                Text('Connecting to server' + (_retryCount > 0 ? ' (retry ' + _retryCount.toString() + '/' + _maxRetries.toString() + ')' : ''),
          ...
          Container(
            ...
            child: Center(
              child: Padding(
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  const Icon(Icons.signal_wifi_statusbar_connected_no_internet_4, color: Color(0xFFFF4444), size: 60),
                  const SizedBox(height: 16),
                  const Text('Stream Unavailable', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 8),
                  const Text('This stream may be offline or geo-restricted.', style: TextStyle(color: Colors.white54, fontSize: 13), textAlign: TextAlign.center),
                  const SizedBox(height: 24),
                  SizedBox(
                    width: double.infinity,
                    child: ElevatedButton.icon(
                      style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF00D2FF), foregroundColor: Colors.black, padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
    ...
    Widget _buildInfoPanel() {
      return Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 14),
          Text(widget.channel.name, style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)),
          const SizedBox(height: 4),
          const Text('CricZ TV • Live Sports Streaming', style: TextStyle(color: Colors.white38, fontSize: 12)),
          const Spacer(),
          Row(children: [
            Expanded(
              child: OutlinedButton.icon(
                style: OutlinedButton.styleFrom(foregroundColor: Colors.white60, side: const BorderSide(color: Color(0xFF1E1E1E)), padding: const EdgeInsets.symmetric(vertical: 13), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
                onPressed: () => Navigator.pop(context),
            ...
            Expanded(
              child: ElevatedButton.icon(
                style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF00D2FF), foregroundColor: Colors.black, padding: const EdgeInsets.symmetric(vertical: 13), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
    ...
    Widget _chip(String text, Color color) => Container(
          ...
          decoration: BoxDecoration(color: color.withOpacity(0.12), borderRadius: BorderRadius.circular(6), border: Border.all(color: color.withOpacity(0.35))),
  ...
  class _CricZControls extends StatefulWidget {
    ...
    const _CricZControls({required this.state, required this.channel, required this.isFullscreen, required this.onFullscreen, required this.onBack});
                ...
                child: StreamBuilder<bool>(
                  ...
                  builder: (_, snap) {
                    ...
                    return GestureDetector(
                      ...
                      child: Container(
                        width: 56, height: 56,
                        decoration: BoxDecoration(color: Colors.black45, shape: BoxShape.circle, border: Border.all(color: Colors.white24, width: 1.5)),
              ...
              Positioned(
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: Icon(widget.isFullscreen ? Icons.fullscreen_exit_rounded : Icons.fullscreen_rounded, color: Colors.white, size: 24),
                  ),
                ),
              ),
            ]),
